...
from anodet import (AnodetDataset, Padim, classification,
                    standard_image_transform, to_batch, visualization)


def get_dataloader(dataset_path, cam_name, object_name):
    dataset = AnodetDataset(
        os.path.join(dataset_path, f"{object_name}/train/good/{cam_name}")
    )
...
def model_fit(model, dataloader, distributions_path, cam_name, object_name):
    ...
    torch.save(
        model.mean,
        os.path.join(
            distributions_path, f"{object_name}/{object_name}_{cam_name}_mean.pt"
        ),
    )
    torch.save(
        model.cov_inv,
        os.path.join(
            distributions_path, f"{object_name}/{object_name}_{cam_name}_cov_inv.pt"
        ),
    )
